...
"""
def find_extreme_words(svm_model, model, n=10):
    \"""
    Finds the n words closest to -1, 0, and 1 of the connotative hyperplane.

    Args:
    svm_model (SVC): The trained SVM model.
    model (gensim.models.Word2Vec): The Word2Vec model containing the vocabulary.
    n (int): The number of words to return for each category (-1, 0, 1).

    Returns:
    dict: A dictionary containing the words closest to -1, 0, and 1.
    \"""
    words = list(model.wv.index_to_key)
    distances = {}

    for word in words:
        distance = svm_model.decision_function([model.wv[word]])[0]
        distances[word] = distance

    sorted_words = sorted(distances.items(), key=lambda x: x[1])
    
    closest_to_minus_one = sorted_words[:n]
    closest_to_one = sorted_words[-n:]
    closest_to_zero = sorted(sorted_words, key=lambda x: abs(x[1]))[:n]

    return {
        'closest_to_minus_one': [word for word, _ in closest_to_minus_one],
        'closest_to_zero': [word for word, _ in closest_to_zero],
        'closest_to_one': [word for word, _ in closest_to_one]
    }
"""

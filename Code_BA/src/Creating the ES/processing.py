...
def process_csv_in_chunks(file_path, parent_dir, processed_folder):
    """
    Process raw data CSV file, removing URLs and splitting into lines of 10 words each.
...
def load_and_process_csv_files(root_folder):
    ...
    csv_files_found = False  # Flag to check if any CSV files are found
    ...
    for dirpath, _, filenames in os.walk(root_folder):
        ...
        for file in filenames:
            ...
            if file.endswith('.csv'):
                csv_files_found = True  # Set the flag when a CSV file is found
        ...
        if not csv_files_found:
            print("Warning: No CSV files were found in the specified root folder.")
